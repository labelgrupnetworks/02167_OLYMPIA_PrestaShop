...
$.formUtils.addValidator({
    ...
    validatorFunction: function (value, $el, config, language, $form, eventContext) {
        ...
        if ($form.find('[data-field-name="id_country"]').length > 0) {
            ...
            if ($form.attr('id') == 'form_address_delivery') {
        ...
        var pattern = countriesJS[id_country]['zip_code_format'];
        ...
        else
        {
            ...
            format = format.replace(/C/g, countriesJS[id_country]['iso_code']);
            ...
            var replacements = {
                ...
                'N': '[0-9]',
                'L': '[a-zA-Z]',
                'C': countriesJS[id_country]['iso_code'],
            };
    ...
    validatorFunction: function (value, $el, config, language, $form, eventContext) {
        ...
        switch (iso_code_country) {
            ...
            case 'CL':
                ...
                break;
            case 'IT':
                let validateVatNumber = function() {
                    var reg = /^[itIT]{2}[0-9]{11}$/;
                    var reg2 = /^[0-9]{11}$/;

                    return (reg.test(value) || reg2.test(value));
                };

                let validateCodiceFiscale = function() {
                    var cf = value.toUpperCase();
                    var cf_reg = /^[A-Z]{6}\d{2}[A-Z]\d{2}[A-Z]\d{3}[A-Z]$/;
                    if (!cf_reg.test(cf)) {
                        return false;
                    }

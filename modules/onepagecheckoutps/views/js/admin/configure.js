...
var AppOPC = {
    ...
    initializeCreditsTraductions: function() {
        $('#helper-credits-developed').html(Msg.credits_developed);
        $('#helper-credits').html(Msg.credits);
        $('#helper-credits-spanish').html(Msg.credits_spanish);
        $('#helper-credits-english').html(Msg.credits_english);
        $('#helper-credits-development').html(Msg.credits_development);
        $('#helper-credits-information').html(Msg.credits_information);
        $('#helper-credits-support').html(Msg.credits_support);
        $('#helper-credits-website').html(Msg.credits_website);
        $('#helper-credits-suggestions').html(Msg.credits_suggestions);
    },
    registerEvents: function(){
        ...
        $button.appendTo($sub_div_add_ip);
        ...
        $('div.tab-content div#tab-checkout50General form div#container-ip_checkout_beta')
            .append($sub_div_add_ip);
        ...
        if (OnePageCheckoutPS.ctopc) {
            var $general_tab = $('#tab-general');
            var $lst_default_group = $general_tab.find('#lst-default_group_customer');
            var $lst_groups_additional = $general_tab.find('#lst-groups_customer_additional');

            $lst_groups_additional.prop('disabled', true);
            $lst_default_group.prop('disabled', true);

            var $small = $('<small>').css('color', 'red').html(Msg.ctopc_enable);
            $lst_groups_additional.parent().append($small);
            $lst_default_group.parent().append($small.clone());
        }

        $('#refresh_statistics').on('click', AppOPC.refreshStatistics);
    },
    addNewIP: function(event) {
        var $txt_ip_debug = $(event.currentTarget).parents('.form-group').find('input[type=text]');
    ...
    fillRequiredFieldList: function() {
        $.getList('table-required-fields', 'getRequiredFieldList', {}, function(result) {
            var $tbody = $('#table-required-fields').children('tbody');
            $.each(result.content, function(i, content) {
                if (content.object === 'customer' && (content.name === 'firstname' || content.name === 'lastname')) {
                    $tbody.find('tr#field_'+content.id).find('td:eq(6)').empty();
                } else if (content.name === 'id_state' || content.name === 'postcode') {
                    var require_message = Msg.info_require_id_state_field;
                    var active_message = Msg.info_active_id_state_field;
                    if (content.name === 'postcode') {
                        require_message = Msg.info_require_postcode_field;
                        active_message = Msg.info_active_postcode_field;
                    }

                    var require_popover_content = '<span type="button" class="btn-popover pts-tooltip" data-toggle="button tooltip" title="'+require_message+'">\n\
                            <i class="fa-pts fa-pts-question-circle nohover text-warning"></i>\n\
                        </span>';
                    $tbody.find('tr#field_'+content.id).find('td:eq(5)').html(require_popover_content);

                    var active_popover_content = '<span type="button" class="btn-popover pts-tooltip" data-toggle="button tooltip" title="'+active_message+'">\n\
                            <i class="fa-pts fa-pts-question-circle nohover text-warning"></i>\n\
                        </span>';
                        $tbody.find('tr#field_'+content.id).find('td:eq(6)').html(active_popover_content);
                } else if (content.name === 'dni') {
                    var dni_popover_content = '<span type="button" class="btn-popover pts-tooltip" data-toggle="button tooltip" title="'+Msg.info_dni_field+'">\n\
                            <i class="fa-pts fa-pts-question-circle nohover text-warning"></i>\n\
                        </span>';

                    $tbody.find('tr#field_'+content.id).find('td:eq(5)').append(dni_popover_content);
                    $tbody.find('tr#field_'+content.id).find('td:eq(6)').append(dni_popover_content);
                }
            });

            $tbody.find('.btn-popover').tooltip();
        });
    },
        ...
        var _json = {
            ...
            success: function(json) {
                if (json.message_code === SUCCESS_CODE ) {
                    OnePageCheckoutPS.SOCIAL_DATA = json.social_data;
                    AppOPC.initStatistics();
                }
            }
        };
...
var Fields = {
    ...
    update: function(event) {
        ...
        if (name === 'id_country' && $.isEmpty(default_value)) {
            errors.push(Msg.required_default_country);
            valid = false;
        }
        ...
        var $container_description = $('#container-field_description');
        var default_description = $container_description.find('#field_description_'+id_language_default).val();
        if ($.isEmpty(default_description)) {
            $container_description.addClass('has-error');
            errors.push(Msg.required_description);
            valid = false;
        } else if($container_description.hasClass('has-error')) {
            $container_description.removeClass('has-error');
        }
...
var SocialNetwork = {
    init: function() {
        SocialNetwork.tab = $('div.tab-content').find('#tab-checkout50SocialNetwork');

        if (SocialNetwork.tab.length > 0) {
            SocialNetwork.form = SocialNetwork.tab.find('form');
            SocialNetwork.form.slideUp();

            SocialNetwork.form.find('#lst-name').on('change', function(event) {
                const nameSocialNetwork = $(event.currentTarget).val();

                if (nameSocialNetwork !== '') {
                    SocialNetwork.form.find('.social-networks').hide();

                    SocialNetwork.form.find('#network-' + nameSocialNetwork).show();
                    SocialNetwork.form.find('.form-group.enabled').show();
                    SocialNetwork.form.find('.form-group.actions').show();
                } else {
                    SocialNetwork.clearForm();
                }
            });

            SocialNetwork.tab.find('#btn-socialNetwort-save').on('click', SocialNetwork.add);
            SocialNetwork.tab.find('#btn-socialNetwort-cancel').on('click', SocialNetwork.hideForm);

            SocialNetwork.tab.find('.btn-socialNetwort-reload').on('click', SocialNetwork.getList);
            SocialNetwork.tab.find('.btn-socialNetwort-add').on('click', function() {
                SocialNetwork.clearForm();
                SocialNetwork.showForm();
            });

            SocialNetwork.getList();
            SocialNetwork.clearForm();
        }
    },
    add: function() {
        const name = SocialNetwork.form.find('#lst-name').val();
        const enabled = SocialNetwork.form.find('#chk-enabled').is(':checked');
        const keys = {};

        const $contentKeys = SocialNetwork.form.find('#network-' + name);

        $.each($contentKeys.find('input'), function(i, item) {
            keys[$(item).attr('name')] = $(item).val();
        });
        ...
        var _json = {
            ...
            success: function(json) {
                if (json.message_code === PresTeamShop.success_code) {
                    SocialNetwork.getList();
                    SocialNetwork.hideForm();
                }
            }
        };
        $.makeRequest(_json);
    },
    delete: function(event, params) {
        if (!confirm(OnePageCheckoutPS.Msg.confirm_delete)) {
            return false;
        }
    ...
    showForm: function() {
        SocialNetwork.tab.find('form').slideDown();
    },
    clearForm: function() {
        SocialNetwork.form.trigger('reset');

        SocialNetwork.form.find('.social-networks').hide();
        SocialNetwork.form.find('.form-group.enabled').hide();
        SocialNetwork.form.find('.form-group.actions').hide();
    },
    hideForm: function() {
        SocialNetwork.form.slideUp();
        SocialNetwork.clearForm();
    ...
    getList : function() {
        $.getList('table-form-list-socialNetwort', 'getSocialNetworkList', {});
    }
};
...
window.addEventListener('load', function(event) {
    AppOPC.init();
    globalTabs.ini();
    AppOPC.initializeCreditsTraductions();
});
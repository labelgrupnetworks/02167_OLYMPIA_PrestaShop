...
{/if}
<input type="hidden" id="rc_page" name="rc_page" value="{$rc_page|escape:'htmlall':'UTF-8'}" />

{if !$register_customer or ($register_customer and !$OnePageCheckoutPS.IS_LOGGED) or ($register_customer and !$rc_page) or ($rc_page eq 'identity')}
    {if isset($OPC_FIELDS[$OPC_GLOBALS->object->customer])}
        <h5 class="onepagecheckoutps_p_step onepagecheckoutps_p_step_one clearfix">
            ...
            {if !$OnePageCheckoutPS.IS_LOGGED or $OnePageCheckoutPS.IS_GUEST}
                <button type="button" id="opc_show_login" class="btn btn-primary btn-sm float-lg-right" >
        ...
        {if !$OnePageCheckoutPS.IS_LOGGED and !$OnePageCheckoutPS.IS_GUEST and isset($amazonpay_opc)}
            <div class="amazonPayButton {if $register_customer}amazonLogin{/if}" data-placement="Other" data-rendered="0" data-color="{$amazonpay_color_button|escape:'htmlall':'UTF-8'}"></div>
        {/if}

        {if !$OnePageCheckoutPS.IS_LOGGED && $opc_social_networks && ($opc_social_networks->facebook->enable || $opc_social_networks->google->enable || $opc_social_networks->paypal->enable || $opc_social_networks->biocryptology->enable)}
            <section id="opc_social_networks">
                ...
                {foreach from=$opc_social_networks key='name' item='network'}
                    {if $network->enable && $network->client_id neq '' && $network->client_secret neq ''}
                        <button type="button" class="btn btn-sm btn-{$name|escape:'html':'UTF-8'}" onclick="Fronted.openWindow('{$link->getModuleLink('onepagecheckoutps', 'login', ['sv' => $network->name_network, 'ajax' => true])|escape:'htmlall':'UTF-8'}', true)">
                            {if $network->name_network eq 'Google'}
                                <img src="{$OnePageCheckoutPS.ONEPAGECHECKOUTPS_IMG|escape:'html':'UTF-8'}social/icon_google.png" alt="google">
...
{*                {/if}*}
                ...
                {if isset($hook_create_account_form)}
                ...
                <div id="delivery_address_container" class="collapse in show">
                    ...
                    <form id="form_address_delivery" autocomplete="on" style="display: none;">
                        <div class="fields_container">
                            ...
                            {if $OnePageCheckoutPS.IS_LOGGED}
                            <div id="action_address_delivery" class="row ">
                ...
                <div id="invoice_address_container" class="collapse in show">
                    ...
                    <form id="form_address_invoice" autocomplete="on" style="display: none;">
                        <div class="fields_container">
                            ...
                            {if $OnePageCheckoutPS.IS_LOGGED}
                            <div id="action_address_invoice" class="row">
    ...
    <div id="div_privacy_policy">
        <p id="p_privacy_policy">
            <label for="privacy_policy">
                <input type="checkbox" class="not_unifrom not_uniform" name="privacy_policy" id="privacy_policy" value="1" {if $privacy_policy}checked="true"{/if} />
...
{if (!$OnePageCheckoutPS.IS_LOGGED && $CONFIGS.OPC_SHOW_BUTTON_REGISTER) or ($register_customer and !$rc_page)}
    <button type="button" id="btn_save_customer" class="btn btn-primary btn-lg btn-block" style="display: none;">
        <i class="fa-pts fa-pts-save fa-pts-lg"></i>
        {l s='Save information' mod='onepagecheckoutps'}
    </button>
{/if}

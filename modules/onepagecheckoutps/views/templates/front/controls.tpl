...
<div id="field_{if $field->object neq ''}{$field->object|escape:'htmlall':'UTF-8'}_{/if}{$field->name|escape:'htmlall':'UTF-8'}"
     class="form-group col-xs-{$num_col|escape:'htmlall':'UTF-8'} col-{$num_col|escape:'htmlall':'UTF-8'} {if $field->required}required{/if} {if $cant_fields == 1}clear clearfix{/if}" {if !$field->active}style="display: none;"{/if}>
    {if $field->type_control eq $OPC_GLOBALS->type_control->textbox}
        ...
        <input
            ...
            type="{if $OPC_GLOBALS->type->{$field->type} eq 'password' or $field->name == 'conf_passwd'}password{elseif $OPC_GLOBALS->type->{$field->type} eq 'email' or $field->name_control eq 'email'}email{else}text{/if}"
            ...
            data-active="{$field->active|intval}"
            {if $field->name == 'address' && $CONFIGS.OPC_AUTOCOMPLETE_GOOGLE_ADDRESS}
                autocomplete="off"
            {elseif $field->name eq 'conf_passwd' or $field->name eq 'passwd'}
                autocomplete="new-password"
            {elseif $field->name eq 'current_passwd'}
        ...
        <div class="row">
            {foreach from=$field->options.data item='item' name='f_options'}
                ...
                <div class="col-xs-{$num_col_option|intval} col-{$num_col_option|intval}">
                    <label for="{$field->name_control|escape:'htmlall':'UTF-8'}">
                        <input
                            id="{$field->id_control|intval}_{$item[$field->options.value]|escape:'htmlall':'UTF-8'}"
    ...
    {elseif $field->type_control eq $OPC_GLOBALS->type_control->textarea}
        <label for="{$field->name_control|escape:'htmlall':'UTF-8'}">
            {$field->description|escape:'htmlall':'UTF-8'}:
            <sup>{if $field->required|intval}*{/if}</sup>
        </label>
        <textarea
            id="{$field->id_control|intval}"
            name="{$field->name_control|intval}"
            class="{$field->classes|escape:'htmlall':'UTF-8'} form-control input-sm not_unifrom not_uniform {if $field->is_custom}custom_field{/if}"
            data-field-name="{$field->name|escape:'htmlall':'UTF-8'}"

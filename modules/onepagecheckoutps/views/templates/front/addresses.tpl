...
{if $cps}
    <div class="col-xs-12 {if !$cps_show_address_message}cps_hide{/if}" id="cps_address_message">
        <div class="alert alert-warning">
            {l s='The selected address will not be taken into account when creating the order because you will pick it up at the store' mod='onepagecheckoutps'}
        </div>
    </div>
{/if}
<div id="address_card_new" class="address_card col-12 col-sm-12 col-md-12 col-lg-12">
...
{if isset($addresses)}
    {if $addresses|count > 4}
        <div id="search_addresses" class="col-md-12 col-12">
    ...
    <div id="address_card_{$address.id_address|intval}" data-id-address="{$address.id_address|intval}" class="address_card col-xs-12 col-md-12 {if $addresses|count > 1}col-lg-6{else}col-lg-12{/if}">
        <div class="container_card {if $address.id_address eq $id_address}selected alert alert-info{/if}">
            ...
            <ul>
                <li>{$address.formatted nofilter}</li>
            </ul>
                {* <ul>

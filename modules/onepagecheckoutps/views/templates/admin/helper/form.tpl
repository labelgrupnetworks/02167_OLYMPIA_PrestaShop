...
{elseif $option.type eq $global->type_control->radio}
        <div class="col-xs-{if isset($option.tooltip)}10{else}12{/if} col-sm-{if isset($modal) and $modal}9{elseif isset($depend) and $depend}4{else}4{/if} nopadding {if isset($modal) and $modal}input-sm{/if} input-group-sm">
            {foreach from=$option.data key="key" item="item"}
                {if isset($option.option_value)}
                    {assign var='value' value=$item[$option.option_value]}
                {else}
                ...
                <div class="radio radio-inline">
                    <label>
                        <input type="radio" name="{$option.name|escape:'htmlall':'UTF-8'}" id="{$option.prefix|escape:'htmlall':'UTF-8'}-{$option.name|escape:'htmlall':'UTF-8'}-{$key|intval}" value="{$value|escape:'htmlall':'UTF-8'}" {if $option.default_option eq $value}checked{/if}>
...
{*                <div class="row form-group clearfix clear" style="margin-top: 5px;">*}
                    {if isset($depend.label) or isset($depend.tooltip)}
                        ...
                        <div class="nopadding-xs
                            ...
                            <label class="col-xs-12 nopadding control-label{if not isset($depend.label)} text-left{/if}" title="{if isset($depend.label)}{$depend.label|escape:'quotes':'UTF-8'}{/if}">

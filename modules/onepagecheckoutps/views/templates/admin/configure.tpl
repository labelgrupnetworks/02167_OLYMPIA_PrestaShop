        ...
        <div role="alert" class="alert alert-info clearfix">
            <div class="col-xs-9 pts-nowrap col-sm-6 col-md-4 nopadding-xs">
                <label class="pts-label-tooltip col-xs-12 nopadding control-label">
                    {l s='Choose a store' mod='onepagecheckoutps'}
                </label>
            </div>

            <div class="col-xs-12">
                <p>
                    {l s='To setup module please you must be choose a store' mod='onepagecheckoutps'}
                </p>
            </div>
        </div>
    </div>
{else}
    <div id="pts_content" class="pts bootstrap nopadding clear clearfix">
        <div class="pts-overlay" style="display: block;"></div>
            ...
            <div class="row">
                {if $paramsBack.isCheckoutBetaAvailable and !$paramsBack.isCheckoutBetaEnabled}
                    <br class="clearfix"/>
                    <div class="col-xs-12">
                        <div id="alert_new_checkout" class="alert alert-info">
                            {l s='Now you can try our new One Page Checkout PS v5.0 with minimalist designs and excellent performance' mod='onepagecheckoutps'}
                            <br/>
                            <a href="#checkout50General" onclick="$('a[href=#tab-checkout50General]').trigger('click');">
                                {l s='Try now!' mod='onepagecheckoutps'}
                            </a>
                        </div>
                    </div>
                {/if}
                {if Shop::isFeatureActive()}
                    ...
                    <div class="clearfix col-xs-12">
                        <div class="alert alert-warning">
                            {if $paramsBack.context_shop}
                                {l s='You are changing the store settings %s, the changes made do not apply to the other stores' sprintf=Context::getContext()->shop->name mod='onepagecheckoutps'}
                            {else}
                                {l s='You are configuring the options of all stores, these changes will not be applied for stores that you have configured individually' mod='onepagecheckoutps'}
                            {/if}
                        </div>
                    </div>
                {/if}

                {if is_array($paramsBack.restricted_groups) and count($paramsBack.restricted_groups) > 0}
                    <br class="clearfix"/>
                    <div class="col-xs-12">
                        <div class="alert alert-warning">
                            <p>
                                {l s='The following customer groups cannot access the module. Modify the permissions from the customer configuration:' mod='onepagecheckoutps'}
                                <a href="{$paramsBack.admin_groups|escape:'htmlall':'UTF-8'}" target="_blank"><i class="fa-pts fa-pts-external-link"></i></a>
                            </p>
                            <ul>
                                {foreach from=$paramsBack.restricted_groups item=group}
                                    <li>{$group.name|escape:'htmlall':'UTF-8'}</li>
                                {/foreach}
                            </ul>
                        </div>
                    </div>
                                ...
                                <div class="tab-content">
                                        ...
                                        {if isset($paramsBack.HELPER_FORM.forms) and is_array($paramsBack.HELPER_FORM.forms) and count($paramsBack.HELPER_FORM.forms)}
                                            {foreach from=$paramsBack.HELPER_FORM.forms key='key' item='form' name='forms'}
                                                ...
                                                <div class="tab-pane {if (isset($CURRENT_FORM) && $CURRENT_FORM eq $form.tab) || (not isset($CURRENT_FORM) && $smarty.foreach.forms.first)}active{/if}" id="tab-{$form.tab|escape:'htmlall':'UTF-8'}">
                                                    {if $form.tab eq 'checkout50General'}
                                                        <div class="alert alert-info">
                                                            {*Para el equipo PresTeamShop es muy emocionante contarte de la nueva generación de nuestro módulo One Page Checkout PS.
                                                            Día a día estamos trabajando muy duro para ir avanzando en esta nueva versión y traerte cada vez más funcionalidades adaptadas de la anterior versión.
                                                            Sientase libre de probar el módulo sin afectar su tienda, ya que solo va a funcionar para las IP que usted habilite.
                                                            Cualquier duda, sugerencia o problema, puede contactarnos y con gusto le atenderemos.*}
                                                            <p>
                                                                For the PresTeamShop team it is very exciting to tell you about the new generation of our One Page Checkout PS module.
                                                            </p>
                                                            <br/>
                                                            <p>
                                                                Day by day we are working very hard to advance in this new version and bring you more and more functionalities adapted from the previous version.
                                                            </p>
                                                            <br/>
                                                            <p>
                                                                Feel free to try the new version without affecting your store, as it will only work for the IPs that you enable.
                                                            </p>
                                                            <br/>
                                                            <p>
                                                                Any questions, suggestions or problems, you can contact us and we will gladly assist you.
                                                            </p>
                                                        </div>
                                                    {/if}
                                                    ...
                                                    {if isset($form.list) and is_array($form.list) and count($form.list)}
                                                        {if isset($form.list.headers) and is_array($form.list.headers) and count($form.list.headers)}
                                                            {if $form.tab eq 'required_fields'}
                                                                {if $paramsBack.ctopc}
                                                                    <div class="alert alert-warning">
                                                                        {l s='The Customer Type OPC module is installed and activated in the store, the configuration saved in this tab will not be taken into account. You must make the configuration from the Customer Type OPC module.' mod='onepagecheckoutps'}
                                                                    </div>
                                                                {/if}
                                                ...
                                                <div class="col-xs-12 col-sm-6 col-md-4 clearfix nopadding-xs">
                                                    <div class="panel clearfix panel-primary">
                                                        ...
                                                        <div class="panel-body nopadding">
                                                            <div id="carrier_{$carrier.id_reference|escape:'htmlall':'UTF-8'}" data-id-reference="{$carrier.id_reference|escape:'htmlall':'UTF-8'}" class="carrier_container">
                                                                {foreach from=$paramsBack.PAYMENTS item='payment'}
                                                                    <label class="checkbox-inline">
                                                                        <input type="checkbox" id="payment_{$payment.id_module|intval}_{$carrier.id_reference|escape:'htmlall':'UTF-8'}" data-id-module="{$payment.id_module|intval}" data-name-payment-module="{$payment.name|escape:'htmlall':'UTF-8'}">&nbsp;{$payment.name|escape:'htmlall':'UTF-8'}
                                                                    </label>
                                                                {/foreach}
                                                            </div>
                                                        </div>
                                    ...
                                    <div class="tab-pane{if (isset($CURRENT_FORM) && $CURRENT_FORM eq 'fields_position')} active{/if}" id="tab-fields_position">
                                        <div class="row">
                                            <div class="col-xs-12">
                                                <div class="alert alert-info">
                                                    {l s='We recommend placing the password field in a single row, as this displays an additional field for confirmation. We make the same recommendation of the password field for the email field if you have the option "Request confirmation email" activated' mod='onepagecheckoutps'}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="tab-pane{if (isset($CURRENT_FORM) && $CURRENT_FORM eq 'pay_methods')} active{/if}" id="tab-pay_methods">
                                        ...
                                        <div id="payment-images-container" class="col-xs-12">
                                            {foreach from=$paramsBack.PAYMENTS_TO_CONFIGURE item='payment'}
                                                                        ...
                                                                        <form class="form-horizontal" role="form">
                                                                            ...
                                                                            <div class="form-group row">
                                                                                ...
                                                                                <div class="col-xs-12 col-md-9 pts-image-change-container">
                                                                                    ...
                                                                                    <img class="img-thumbnail" id="image_payment_{$payment.id_module|intval}" height="49" width="86" src="{if file_exists($filename)}{$path_image|escape:'htmlall':'UTF-8'}{else}{$filename_default|escape:'htmlall':'UTF-8'}{/if}?{$smarty.now|escape:'htmlall':'UTF-8'}" />
                                                                                    <br />
                                                                                    <a href="#" id="remove-image-handler-{$payment.id_module|intval}" class="pts-remove-image-handler col-xs-12 nopadding {if !$payment.show_delete_button || $payment.name_image eq 'no-image.png'}hidden{/if}" data-id-module="{$payment.id_module|intval}" data-name-module="{$payment.name|escape:'htmlall':'UTF-8'}">
                                                                                        <i class="fa-pts fa-pts-remove"></i>
                                                                                        {l s='Remove image' mod='onepagecheckoutps'}</span>
                                                                                    </a>
                                                                                    <a href="#" id="change-image-handler-{$payment.id_module|intval}" class="pts-change-image-handler col-xs-12 nopadding" data-id-module="{$payment.id_module|intval}" data-name-module="{$payment.name|escape:'htmlall':'UTF-8'}">
                                                                                        <i class="fa-pts fa-pts-refresh"></i>
                                                                                        {l s='Upload' mod='onepagecheckoutps'}&nbsp;<span class="pts-change-image-name">{l s='image' mod='onepagecheckoutps'}</span>
                                                                                    </a>

                                                                                    <input id="file-image_payment-{$payment.id_module|intval}" data-name-module="{$payment.name|escape:'htmlall':'UTF-8'}" type="file" class="hidden">
                                                                                </div>
                                                                                <div class="clear clearfix"></div>
                                                                            </div>
                                                                            <div class="form-group row">
                                                                                <div class="col-xs-12 col-md-6 col-lg-4 col-md-push-3">
                                                                                    <button id="btn-save_image_payment-{$payment.name|escape:'htmlall':'UTF-8'}" data-id-module="{$payment.id_module|intval}" data-name-module="{$payment.name|escape:'htmlall':'UTF-8'}" class="btn btn-primary btn-block save-image-payment has-action">
                                                                                        <i class="fa-pts fa-pts-save nohover"></i>
                                                                                        {l s='Save' mod='onepagecheckoutps'}
                                                                                    </button>
                                                                                </div>
                                        ...
                                        <div class="row">
                                            {if is_array($paramsBack.SOCIAL_DATA) && count($paramsBack.SOCIAL_DATA) > 0}
                                                ...
                                                <div class="col-md-3 col-md-offset-4 text-center">
                                                    <button id="refresh_statistics" class="btn btn-primary"><i class="fa-pts fa-pts-refresh"></i> {l s='Refresh' mod='onepagecheckoutps'}</button>
                                                </div>
                                            {else}
                                    ...
                                    {include file=$paramsBack.MODULE_TPL|cat:'views/templates/admin/tabs/social_network.tpl'}
        ...
        {if file_exists($paramsBack.MODULE_TPL|cat:'views/templates/admin/helper/credits.tpl')}
            {include file=$paramsBack.MODULE_TPL|cat:'views/templates/admin/helper/credits.tpl'}
        {else}
            {include file=$paramsBack.MODULE_TPL|cat:'views/templates/admin/helper/credits_addons.tpl'}
        {/if}
    </div>
{/if}

...
class OPCCore
{
        ...
        if (Shop::isFeatureActive() && $id_shop > 0) {
            ...
            foreach ($shops as $shop) {
                Configuration::updateValue(
                    $this->module->prefix_module.'_VERSION',
                    $this->module->version,
                    false,
                    null,
                    $shop['id_shop']
    ...
    public function getRegisteredVersion()
    {
        ...
        if (empty($registered_version) && Shop::isFeatureActive()) {
            $query = new DbQuery();
            $query->select('value');
            $query->from('configuration');
            $query->where('name = \''.$this->module->prefix_module.'_VERSION\' AND value IS NOT NULL');

            $registered_version = Db::getInstance()->getValue($query);
        }
    ...
    public function getContentUpgrade()
    {
        ...
        //---------------------------------------------------------------------------
        $registered_version = $this->getRegisteredVersion();
        if (!empty($registered_version) && $registered_version != $this->module->version) {
            ...
            if ($overrides) {
                ...
                $this->context->smarty->assign(array(
                    ...
                    'message_choose_all_stores' => $this->module->l('Choose all stores', self::NAME_CLASS),
                    'message_all_stores' => $this->module->l('You need to update the version of our module for all your stores, please select the option "All stores" to perform this process', self::NAME_CLASS)
                ));
            }

            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->module->name
                .'/views/templates/admin/helper/update_version.tpl');
        }
    ...
    public function searchOverrides()
    {
        ...
        if (file_exists(_PS_OVERRIDE_DIR_.'modules/'.$this->module->name.'/'.$this->module->name.'.php')) {
            $md5_file = $this->createMD5OfFileContent(_PS_MODULE_DIR_.$this->module->name.'/'
                .$this->module->name.'.php');
            $md5_override = $this->createMD5OfFileContent(_PS_OVERRIDE_DIR_.'modules/'.$this->module->name.'/'
                .$this->module->name.'.php');
    ...
    public function displayForm()
    {
        ...
        $this->module->params_back = array_merge(
            array(
                ...
                'PREFIX_MODULE' => $this->module->prefix_module,
        ...
        if ($registered_version != $this->module->version) {
            ...
            foreach ($list as $num => $file_detail) {
                ...
                if (function_exists($file_detail['upgrade_function'])) {
                    try {
                        $return = $file_detail['upgrade_function']($this->module);
                        if ($return === true) {
                            Configuration::updateValue($this->module->prefix_module.'_VERSION', $file_detail['version']);
                        } elseif ($return === false) {
                            return Tools::jsonEncode(
                                array (
                                    'message' => $this->module->l('There is a problem updating the module, please contact the developers', self::NAME_CLASS)
                                )
                            );
                        } else {
                            return Tools::jsonEncode(array('message' => $return));
                        }
                    } catch (Exception $ex) {
            ...
            $this->saveContentCodeEditors(
                realpath(_PS_MODULE_DIR_.$this->module->name.'/views/css/front/override.css'),
                Configuration::get($this->module->override_css)
            );

            $this->saveContentCodeEditors(
                realpath(_PS_MODULE_DIR_.$this->module->name.'/views/js/front/override.js'),
                Configuration::get($this->module->override_js)
            );
    ...
    {
        if (!$this->checkModulePTS()) {
            return false;
        }

        $display_module = true;

        $enable_debug = Configuration::get($this->module->prefix_module.'_ENABLE_DEBUG');
        if ($enable_debug) {
            $display_module = false;
            $my_ip = Tools::getRemoteAddr();
            $ip_debug = Configuration::get($this->module->prefix_module.'_IP_DEBUG');
    ...
    public function isModuleActive($name_module, $function_exist = false)
    {
        if (Module::isInstalled($name_module)) {
            ...
            if (Validate::isLoadedObject($module) && $module->active) {
                if ($name_module === $this->module->name) {
                    if (!$this->isVisible()) {
                        return false;
                    }
                }
    ...
    {
        $content = Tools::getValue('content', $content);
        $filepath = urldecode(Tools::getValue('filepath', $filepath));
    ...
    public function saveTranslations($iso_code)
    {
        ...
        if (is_writable($file_path)) {
            ...
            return array(
                'message_code' => $this->CODE_ERROR,
                'message' => $this->module->l('You do not have write permission in the translations folder', self::NAME_CLASS)
            );
        }

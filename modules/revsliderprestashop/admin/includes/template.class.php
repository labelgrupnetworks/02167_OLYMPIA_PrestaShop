...
class RevSliderTemplate extends RevSliderFunctions {
	...
	public function _download_template($uid){
		...
		if(RevLoader::wp_mkdir_p($upload_dir['basedir'].$this->templates_path)){ //check here to not flood the server
			$data = array(
				'code'		=> urlencode($code),
			...
			$request = RevLoader::wp_remote_post($url, array(
				...
				if($response = $this->get_val($request, 'body')){
					...
					if($response !== 'invalid'){
						...
						if($ret !== false){
							...
							$return = array('error' => RevLoader::__('Can\'t write the file into the uploads folder of WordPress, please change permissions and try again!', 'revslider'));
						}
					}else{
						$error = ($this->get_addition('selling') === true) ? RevLoader::__('License Key is invalid', 'revslider') : RevLoader::__('Purchase Code is invalid', 'revslider');
						
						$return = array('error' => $error);
					}
				}
			}else{//else, check for error and print it to customer
				$return = array('error' => RevLoader::__('Can\'t connect programatically to the ThemePunch servers, please check your webserver settings', 'revslider'));
			}
		}else{
			$return = array('error' => RevLoader::__('Can\'t write into the uploads folder of WordPress, please change permissions and try again!', 'revslider'));
		}
	...
	// 		//track
	// 		// $url = RevLoader::url() . 'download.php?code='. $data['code'].'&type=templates&uid='.$data['uid'] .'&product='. $data['product'];
	// 		// $request = RevLoader::wp_remote_post($url, array(
	// 		// 	'user-agent' => 'php/; '. RevLoader::get_bloginfo('url'),
	// 		// 	'body' => '',
	// 		// 	'timeout' => 400
	// 		// ));
	// 		// $request = RevLoader::wp_remote_post($this->templates_url.$this->templates_download, array(
	// 		// 	'user-agent' => 'php/; '. RevLoader::get_bloginfo('url'),
	// 		// 	'body' => array(
	// 		// 		'code' => urlencode($code),
	// 		// 		'shop_version' => urlencode(self::SHOP_VERSION),
	...
	public function _get_template_list($force = false){
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
			...
			// 			if(isset($templates['hash'])) RevLoader::update_option('revslider-templates-hash', $templates['hash']);
			// 			RevLoader::update_option('rs-templates-new', $templates, false);
			// 		}
			// 	}
			// }
	...
	private function update_template_list(){
		...
		$cur = RevLoader::get_option('rs-templates', array());
		$counter = 0;
	...
	private function _update_images($img = false){
		$rslb	= new RevSliderLoadBalancer();
		$templates = RevLoader::get_option('rs-templates', array());
		//track
						...
						if(!file_exists($file) || isset($temp['push_image'])){
							if($curl !== false){
								...
								do{
									...
									$image_data = RevLoader::wp_remote_post($url.'/'.$this->templates_server_path.$temp['img'], array(
								...
								if(!file_exists($file) || isset($reload[$key])){ //update, so load again
									if($curl !== false){
										...
										do{
											...
											$image_data = RevLoader::wp_remote_post($url.'/'.$this->templates_server_path.$tvalues['img'], array(
												'user-agent' => 'revphp; '. RevLoader::get_bloginfo('url'),
												'body'		 => array(
													'item'		=> urlencode(RS_PLUGIN_SLUG),
													'version'	=> urlencode(RS_REVISION)
	...
	public function get_tp_template_sliders($uid = false){
		...
		$defaults = RevLoader::get_option('rs-templates', array());
		...
		if(!empty($defaults)){
			...
			foreach($defaults as $dk => $default){
				...
				if(!empty($defaults[$dk]['plugin_require'])){
			      ...
			      //commented
						// $plugin_list[$path] = (is_plugin_active( RevLoader::esc_attr($path))) ? true : false;

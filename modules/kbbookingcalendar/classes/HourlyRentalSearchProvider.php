...
class HourlyRentalSearchProvider implements ProductSearchProviderInterface
{
    private $translator;
    ...
    public function runQuery(
        ...
        $count = (int)self::getNbHourlyRentalProduct();
...
//        Tools::dieObject($result, true);
    ...
    public static function getProduct($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        $sql = 'SELECT p.*,  stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,
            ...
            .' FROM '._DB_PREFIX_.'kb_booking_product b 
                INNER JOIN `'._DB_PREFIX_.'product` p ON (b.`id_product` = p.`id_product` AND b.product_type="hourly_rental")
     ...
     */
    public static function getNbHourlyRentalProduct()
    {
        $sql = 'SELECT COUNT(ps.`id_product`) AS nb
				FROM `'._DB_PREFIX_.'kb_booking_product` ps
				LEFT JOIN `'._DB_PREFIX_.'product` p ON (p.`id_product` = ps.`id_product` AND ps.product_type="hourly_rental")
				'.Shop::addSqlAssociation('product', 'p', false).'

...
class HotelSearchProvider implements ProductSearchProviderInterface
{
    private $translator;
    ...
    public function runQuery(
        ...
        $count = (int)self::getNbHotelProduct();
    ...
    public static function getProduct($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        $sql = 'SELECT p.*,  stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,	
            ...
            INNER JOIN `'._DB_PREFIX_.'product` p ON (b.`id_product` = p.`id_product` AND b.product_type="hotel_booking")
     ...
     */
    public static function getNbHotelProduct()
    {
        $sql = 'SELECT COUNT(ps.`id_product`) AS nb
				FROM `'._DB_PREFIX_.'kb_booking_product` ps
				LEFT JOIN `'._DB_PREFIX_.'product` p ON (p.`id_product` = ps.`id_product` AND ps.product_type="hotel_booking")
				'.Shop::addSqlAssociation('product', 'p', false).'

...
require_once _PS_MODULE_DIR_ . 'kbbookingcalendar/classes/AppointmentsSearchProvider.php';
...
class KbBookingCalendarAppointmentsModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function initContent()
    {
        ...
        if (!empty($kb_setting) && $kb_setting['enable']) {
            $appointment_products = KbBookingProduct::getProductsByType('appointment');
            if ($this->ajax) {
                ob_end_clean();
                header('Content-Type: application/json');
                $this->ajaxDie(json_encode($this->getAjaxProductSearchVariables()));
            } else {
                $variables = $this->getAppointmentsProducts($appointment_products);
    ...
    protected function getAppointmentsProducts()
    {
        if (Configuration::get('PS_CATALOG_MODE')) {
            return false;
        }

        $searchProvider = new AppointmentsSearchProvider(
...
//        $query->setSortOrder(SortOrder::random());
        ...
        $searchVariables = array(
            'label' => $this->module->l('Appointments', 'appointments'),
    ...
    protected function getTemplateVarSortOrders(array $sortOrders, $currentSortOrderURLParameter)
    {
        return array_map(function ($sortOrder) use ($currentSortOrderURLParameter) {
            $app = new KbBookingCalendarAppointmentsModuleFrontController();
            $order = $sortOrder->toArray();
            $order['current'] = $order['urlParameter'] === $currentSortOrderURLParameter;
            $order['url'] = $app->updateQueryString(array(
    ...
    protected function getTemplateVarPagination(
        ...
        return array(
            ...
            'pages' => array_map(function ($link) {
                $app = new KbBookingCalendarAppointmentsModuleFrontController();
                $link['url'] = $app->updateQueryString(array(
                    'page' => $link['page'],
                ));

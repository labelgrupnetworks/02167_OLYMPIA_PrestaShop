...
require_once _PS_MODULE_DIR_ . 'kbbookingcalendar/classes/HotelSearchProvider.php';
...
class KbBookingCalendarHotelsModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function initContent()
    {
        ...
        if (!empty($kb_setting) && $kb_setting['enable']) {
            $hotel_booking_products = KbBookingProduct::getProductsByType('hotel_booking');
            if ($this->ajax) {
                ob_end_clean();
                header('Content-Type: application/json');
                $this->ajaxDie(json_encode($this->getAjaxProductSearchVariables()));
            } else {
                $variables = $this->getHotelProducts($hotel_booking_products);
    ...
    protected function getHotelProducts()
    {
        if (Configuration::get('PS_CATALOG_MODE')) {
            return false;
        }

        $searchProvider = new HotelSearchProvider(
...
//        $nProducts = (int) 10;
        ...
        $searchVariables = array(
            'label' => $this->module->l('Hotel Booking', 'hotels'),
...
//            'rendered_active_filters' => $rendered_active_filters,
    ...
    protected function getTemplateVarSortOrders(array $sortOrders, $currentSortOrderURLParameter)
    {
        return array_map(function ($sortOrder) use ($currentSortOrderURLParameter) {
            $kbhotel = new KbBookingCalendarHotelsModuleFrontController();
            $order = $sortOrder->toArray();
            $order['current'] = $order['urlParameter'] === $currentSortOrderURLParameter;
            $order['url'] = $kbhotel->updateQueryString(array(
    ...
    protected function getTemplateVarPagination(
        ...
        return array(
            ...
            'pages' => array_map(function ($link) {
                $Kbhotel = new KbBookingCalendarHotelsModuleFrontController();
                $link['url'] = $Kbhotel->updateQueryString(array(
                    'page' => $link['page'],
                ));
    ...
    public function postProcess()
    {
        ...
        if (Tools::getValue('ajax')) {
            if (Tools::getValue('renderHotelRoom')) {
                $id_booking_product = Tools::getValue('id_booking_product');
//                if (Tools::getValue('id_product', 0) > 0) {
                $booking_product = KbBookingProduct::getProductDetailsByID(Tools::getValue('id_product'));
//                }
                
                $id_hotel_room = Tools::getValue('id_hotel_room');
                $hotel_rooms = KbBookingProduct::getHotelProductRoomsByID($id_booking_product, $id_hotel_room);
                ...
                if (!empty($hotel_rooms)) {
                    ...
                    if (!empty($hotel_rooms)) {
                        foreach ($hotel_rooms as &$rooms) {
                            if ($rooms['id_booking_room_facilities_map'] == $id_hotel_room) {
                        ...
                        $this->context->smarty->assign(array(
                            ...
                            'no_img' => $this->getModuleDirUrl() . $this->module->name . '/views/img/404.gif?time=' . time(),
                        ));
                    }
                }
                $data = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module->name . '/views/templates/hook/hotel_rooms.tpl');
                echo $data;
                die;
            }
            if (Tools::getValue('searchHotelRoom')) {
                $id_booking_product = Tools::getValue('id_booking_product');
                ...
                if (!empty($id_product)) {
                    ...
                    if (!empty($booking_product)) {
                        if ($booking_product['id_booking_product'] == $id_booking_product) {
                            if ($booking_product['product_type'] == 'hotel_booking') {
                                $hotel_rooms = KbBookingProduct::getHotelProductRoomsByIDANDCategory($id_booking_product, $room_category);
                                if (!empty($hotel_rooms)) {
                                    foreach ($hotel_rooms as $room_key => $room_data) {
                                        $room_type_obj = new KbBookingRoomType($room_data['id_room_type']);
                                        if ((int)$room_type_obj->active == 0) {
    ...
    //                                        $rooms['price'] = Tools::displayPrice(Tools::convertPriceFull($rooms['price'], Currency::getDefaultCurrency()));
                                        ...
                                        $this->context->smarty->assign(array(
                                            ...
                                            'searchroomcat' => true,
                                             'no_img' => $this->getModuleDirUrl() . $this->module->name . '/views/img/404.gif?time=' . time(),
                                            'hotel_url' => $this->context->link->getModuleLink($this->module->name, 'hotels'),
                                        ));
                                        $booking_room_tpl = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module->name . '/views/templates/hook/hotel_rooms.tpl');
                                    }
                                }
                            }
                        }

...
require_once _PS_MODULE_DIR_ . 'kbbookingcalendar/classes/DailyRentalSearchProvider.php';
...
class KbBookingCalendarDailyRentalsModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function initContent()
    {
        ...
        if (!empty($kb_setting) && $kb_setting['enable']) {
            $daily_rental_products = KbBookingProduct::getProductsByType('daily_rental');
            if ($this->ajax) {
                ob_end_clean();
                header('Content-Type: application/json');
                $this->ajaxDie(json_encode($this->getAjaxProductSearchVariables()));
            } else {
                $variables = $this->getDailyRentalProducts($daily_rental_products);
    ...
    {
        $search = $this->getDailyRentalProducts();
    ...
    protected function getDailyRentalProducts()
    {
        if (Configuration::get('PS_CATALOG_MODE')) {
            return false;
        }

        $searchProvider = new DailyRentalSearchProvider(
...
//        $nProducts = (int) 10;
        ...
        $searchVariables = array(
            'label' => $this->module->l('Daily Rental', 'dailyrentals'),
    ...
    protected function getTemplateVarSortOrders(array $sortOrders, $currentSortOrderURLParameter)
    {
        return array_map(function ($sortOrder) use ($currentSortOrderURLParameter) {
            $daily = new KbBookingCalendarDailyRentalsModuleFrontController();
            $order = $sortOrder->toArray();
            $order['current'] = $order['urlParameter'] === $currentSortOrderURLParameter;
            $order['url'] = $daily->updateQueryString(array(
    ...
    protected function getTemplateVarPagination(
        ...
        return array(
            ...
            'pages' => array_map(function ($link) {
                $daily = new KbBookingCalendarDailyRentalsModuleFrontController();
                $link['url'] = $daily->updateQueryString(array(
                    'page' => $link['page'],
                ));

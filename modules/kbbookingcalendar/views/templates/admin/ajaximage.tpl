...
<div class="form-group">
    <div class="col-lg-12">
        ...
        <button class="btn btn-default" data-style="expand-right" data-size="s" type="button" id="{$id|escape:'html':'UTF-8'}-add-button">
            <i class="icon-folder-open"></i> {if isset($multiple) && $multiple}{l s='Add files...' mod='kbbookingcalendar'}{else}{l s='Add file...' mod='kbbookingcalendar'}{/if}
        </button>
    </div>
</div>
<div class="well" style="display:none">
    ...
    <button class="ladda-button btn btn-primary" data-style="expand-right" type="button" id="{$id|escape:'html':'UTF-8'}-upload-button" style="display:none;">
        <span class="ladda-label"><i class="icon-check"></i> {if isset($multiple) && $multiple}{l s='Upload files' mod='kbbookingcalendar'}{else}{l s='Upload file' mod='kbbookingcalendar'}{/if}</span>
    </button> 
    {*    <div id="kb-booking-loader"></div>*}
</div>
...
<script type="text/javascript">
    ...
    {if isset($multiple) && isset($max_files)}
        var {$id|escape:'html':'UTF-8'}_max_files = {$max_files - count($files)};
    {/if}
    ...
    {$id|escape:'html':'UTF-8'}_upload_button.stop();
                ...
                if (typeof {$id|escape:'html':'UTF-8'}_max_files !== 'undefined') {
                    if ({$id|escape:'html':'UTF-8'}_total_files >= {$id|escape:'html':'UTF-8'}_max_files) {
                        e.preventDefault();
    {*                        $('#kb-booking-loader').hide();*}
                        alert('{l s='You can upload a maximum of %s files'|sprintf:$max_files mod='kbbookingcalendar'}');
                        return;
                    }
                }
                ...
                var button = $('<button/>').addClass('btn btn-default pull-right').prop('type', 'button').html('<i class="icon-trash"></i> {l s='Remove file' mod='kbbookingcalendar'}').appendTo(data.context).on('click', function () {
    {$id|escape:'html':'UTF-8'}_total_files--;
                    data.files = null;

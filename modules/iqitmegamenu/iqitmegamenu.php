...
');
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddTab')) {
            ...
            if (isset($id_shop_list) && !empty($id_shop_list)) {
                ...
                $tab->id_shop_list[]= Context::getContext()->shop->id;
            }
    ...
    public function renderAddForm()
    {
        ...
        if (Tools::isSubmit('id_tab') && IqitMenuTab::tabExists((int)Tools::getValue('id_tab'))) {
            ...
            if ($tab->submenu_type == 2 && $tab->submenu_content != '') {
                ...
                $arr = Tools::jsonDecode($tab->submenu_content, true);

                if (!is_array($arr)){
                    $arr = array();
                }
                $submenu_content_format = $this->buildSubmenuTree($arr, false);
            }
        }
    ...
    public function generateSubmenuCss($submenu, $parent)
    {
        ...
        foreach ($submenu as $key => $element) {
            ...
            if (isset($element['content_s']['title_color'])) {
                $css .= $parent . ' .menu-element-id-' . $element['elementId'] . ' > .cbp-menu-column-inner a.cbp-column-title, '. $parent . ' .menu-element-id-' . $element['elementId'] . ' > .cbp-menu-column-inner a.cbp-column-title:link{
																color: ' . $element['content_s']['title_color'] . ' !important;
															}' . PHP_EOL;
            }

            if (isset($element['content_s']['title_colorh'])) {
                $css .= $parent . ' .menu-element-id-' . $element['elementId'] . ' > .cbp-menu-column-inner a.cbp-column-title:hover{
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        if($this->context->controller->php_self != 'index'){
            $cacheId = 'iqitmegamenu|index';
        } else{
            $cacheId = 'iqitmegamenu|other';
        }
    ...
    public function renderCategoriesSelect($frontend)
    {
        ...
        $return_categories = $this->generateCategoriesOption2(Category::GetNestedCategories(null,
            (int)$this->context->language->id, false), $frontend);

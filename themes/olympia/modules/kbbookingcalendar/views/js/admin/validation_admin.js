...
//    $('input[name="product_upc"]').removeClass('error_field');
    ...
    if (!date_error) {
        ...
        $('.kb-datetime-row').each(function () {
            ...
            abc.push($(this).find('.kb_weekday').val());
        });
        var i, j, z;
        if (abc.length % 3 !== 0)
            throw new TypeError('Date range length must be a multiple of 2');
        for (i = 0; i < abc.length - 3; i += 3) {
            for (j = i + 3; j < abc.length; j += 3) {
                if (
                    dateRangeWeekdayOverlaps(
                        abc[i], abc[i + 1],
                        abc[j], abc[j + 1], 
                        abc[i + 2], abc[j + 2],
                    )
...
function dateRangeWeekdayOverlaps(a_start, a_end, b_start, b_end, a_weekday, b_weekday) {
    alert('check dates')
    console.log(a_start)
    console.log(a_end)
    console.log(b_start)
    console.log(b_end)
    if (a_start <= b_start && b_start <= a_end)
        if(a_weekday != undefined && a_weekday.length > 0 && b_weekday != undefined && b_weekday.length > 0)
            a_weekday.forEach(a_element => {
                b_weekday.forEach(b_element => {
